...
// --- 2. GRÁFICO (Ahora agrupa directo por Diagnostico) ---
$sql_graf = "
...
// --- 3. ALERTAS ---
// (El bloque de alertas y paginación es idéntico al anterior, pero asegurando el WHERE correcto)
// ... [MANTENER EL CÓDIGO DE PAGINACIÓN DE LA RESPUESTA ANTERIOR] ...
// Para abreviar, aquí pego la consulta clave de alertas:
$sql_base_alertas = "
    FROM Alerta a
...
$sql_alertas = "SELECT a.Id as IdAlerta, a.Estado, e.Nombre as Estudiante, est.Nombre as Establecimiento, r.IMC, r.FechaMedicion, r.Diagnostico " . $sql_base_alertas . $where_a . " ORDER BY a.Estado DESC, r.FechaMedicion DESC LIMIT $registros_por_pagina OFFSET $offset";
...
<body>
    <div class="dashboard-wrapper">
        ...
        <main class="main-content">
            <header class="header"><div class="header-user"><?php echo $_SESSION['user_nombre']; ?></div><a href="logout.php" class="btn-logout">Salir</a></header>
            <section class="content-body">
                <div class="content-container" style="background:transparent; box-shadow:none; padding:0;">
                    ...
                    <div class="top-filters">
                        ...
                        <form style="display:flex; gap:10px; flex-grow:1;">
                            <?php if(isset($_GET['ver'])): ?><input type="hidden" name="ver" value="<?php echo $_GET['ver']; ?>"><?php endif; ?>
                            <select name="colegio" onchange="this.form.submit()" style="padding:8px; border:1px solid #ddd; border-radius:4px;">
                                <option value="">🏢 Todos los Colegios</option>
                                <?php foreach($colegios as $c): ?><option value="<?php echo $c['Id']; ?>" <?php echo $filtro_colegio==$c['Id']?'selected':''; ?>><?php echo $c['Nombre']; ?></option><?php endforeach; ?>
                            </select>
                    ...
                    <div class="charts-container">
                        ...
                        <div class="chart-box">
                            <div style="display:flex; justify-content:space-between; margin-bottom:15px;">
                                <h3>Alertas</h3>
                                <div>
                                    <a href="?ver=pendientes" style="margin-right:10px; font-weight:bold; color:<?php echo $filtro_estado===1?'#0d6efd':'#666'; ?>">Pendientes</a>
                                    <a href="?ver=todos" style="font-weight:bold; color:<?php echo $filtro_estado===null?'#0d6efd':'#666'; ?>">Historial</a>
                                </div>
                            </div>
                            ...
                            <div class="pagination">
                                <?php for($i=1; $i<=$total_pags; $i++): ?>
                                    <a href="?pag=<?php echo $i; ?>&ver=<?php echo $_GET['ver']??'pendientes'; ?>&colegio=<?php echo $filtro_colegio; ?>&sexo=<?php echo $filtro_sexo; ?>" class="page-link <?php echo $i==$pagina_actual?'active':''; ?>"><?php echo $i; ?></a>
                                <?php endfor; ?>
                            </div>
                        </div>
                    </div>
