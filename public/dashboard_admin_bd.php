...
// --- VALIDACIÓN DE SEGURIDAD PARA FILTROS ---
        ...
        try {
            ...
            if ($accion == 'eliminar') {
                ...
                if ($tipo == 'establecimiento') {
                    ...
                    $_SESSION['success_message'] = "Establecimiento eliminado correctamente.";
...
<head>
    ...
    <style>
        ...
        .page-link { 
            ...
            text-decoration: none; border-radius: 4px; color: #333; font-weight: 500; transition: all 0.2s;
        }
...
<body>
    ...
    <header class="main-header">
        <div class="header-left">
            ...
            <div class="brand-logo">
                <i class="fa-solid fa-database"></i> NutriData <span style="font-weight:400; color:#666; font-size:1rem; margin-left:5px;">| Admin BD</span>
            </div>
        </div>
        <div class="header-user-section">
            <div class="user-info">
                <span class="user-name"><?php echo htmlspecialchars($_SESSION['user_nombre']); ?></span>
                <span class="user-role">Administrador Base de Datos</span>
            </div>
    ...
    <aside class="sidebar" id="mainSidebar">
        <div class="sidebar-header">
            <h3 style="color:var(--primary-color); font-size:1.1rem; margin:0;">Menú Principal</h3>
    ...
    <main class="main-content">
        <?php if (isset($_SESSION['success_message'])): ?>
            <div class="mensaje success"><i class="fa-solid fa-check-circle"></i> <?php echo $_SESSION['success_message']; unset($_SESSION['success_message']); ?></div>
        <?php endif; ?>
        
        <?php if (isset($_SESSION['error'])): ?>
            <div class="mensaje error"><i class="fa-solid fa-triangle-exclamation"></i> <?php echo $_SESSION['error']; unset($_SESSION['error']); ?></div>
        <?php endif; ?>
        ...
        <div class="content-container">
            ...
            if ($vista === 'usuarios') {
                ...
                // BREADCRUMBS
                echo '<nav class="breadcrumbs" style="font-size:0.95rem;">
                        <a href="dashboard_admin_bd.php?vista=estudiantes" style="text-decoration:none; color:'.(!$id_establecimiento?'#000; font-weight:bold;':'var(--primary-color)').'">Región Biobío</a>';
                if ($id_establecimiento) {
                    $stmt = $pdo->prepare("SELECT Nombre FROM Establecimiento WHERE Id = ?"); $stmt->execute([$id_establecimiento]);
                    echo ' <span style="color:#ccc">/</span> <a href="dashboard_admin_bd.php?vista=estudiantes&id_establecimiento='.$id_establecimiento.'" style="text-decoration:none; color:'.($id_establecimiento && !$id_curso?'#000; font-weight:bold;':'var(--primary-color)').'">' . htmlspecialchars($stmt->fetchColumn()) . '</a>';
                }
                if ($id_curso) {
                    $stmt = $pdo->prepare("SELECT Nombre FROM Curso WHERE Id = ?"); $stmt->execute([$id_curso]);
                    echo ' <span style="color:#ccc">/</span> <span style="font-weight:bold; color:#000;">' . htmlspecialchars($stmt->fetchColumn()) . '</span>';
                }
                echo '</nav>';
